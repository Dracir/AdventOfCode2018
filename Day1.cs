...
namespace AdventOfCode2018
{
	public class Day1
	{
		...
		struct DataP2
		{
			...
			public bool SeenDuplicate;

			public DataP2(int frequency, List<int> seen, bool seenDuplicate)
			{
				Frequency = frequency;
				Seen = seen;
				SeenDuplicate = seenDuplicate;
			}
		}
		...
		{
			var values = input.Split(",").Select(s => Int32.Parse(s)).ToArray();

			var seed = new DataP2(0, new List<int>(), false);
			var source = WhileTrue(values).Cast<int>();

			var result = source.AggregateUntil(seed, AddTo, dic => dic.SeenDuplicate);
		...
		private static DataP2 AddTo(DataP2 list, int change)
		{
			var newFrequency = list.Frequency + change;
			var newList = list.Seen.ToList();
			bool seenDuplicate = false;
			if (newList.Contains(newFrequency))
				seenDuplicate = true;
			else
				newList.Add(newFrequency);

			return new DataP2(newFrequency, newList, seenDuplicate);
		}
